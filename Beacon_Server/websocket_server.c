...
/* it looks like we need to define 4 protocols, the first is always http    */
/* this tells the program how to serve http files                           */
/* the second is our web sockets protocol that we can name as we see fit    */
/* the third is our restreamer that sends the data back out                 */
/* the forth line is a list terminator and just has a null protocol defined */
static struct lws_protocols protocols[] = {
	{ "http", lws_callback_http_dummy, 0, 0, 0, NULL, 0},
	LWS_PLUGIN_PROTOCOL_BEACON_SERVER,
	LWS_PLUGIN_PROTOCOL_BEACON_RESTREAM,
...
{
	/* in order to set up a websocket, we first create a context structure (called info in this case) */
	...
	/* here we setup the level of loggin we are going to do ... see logs variable declared above */
        lws_set_log_level(logs, NULL);
	/* this logs a message to the user telling them what to do with the program */
        lwsl_user("LWS Meteor Beacon ws server | visit http://localhost:7682 (-s = use TLS / https)\n");

        /* now we are ready to create our   info   structure that we will send into the   create   function */
        memset(&info, 0, sizeof info); /* otherwise uninitialized garbage */
	info.port = 7682;   /* this is the URL port that we are going to set up our socket on */
