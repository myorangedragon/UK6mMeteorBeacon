/*
 * ws protocol handler plugin for "server.c" - restream Protocol
 *
 */
...
const struct lws_protocols *protocol_restreamer;
...
int callback_beacon_restream(struct lws *wsi, enum lws_callback_reasons reason,
		 	                 void *user, void *in, size_t len)
{
	...
	switch (reason) {
      ...
      case LWS_CALLBACK_PROTOCOL_INIT:
		printf("Restream WS INIT\n");
                protocol_restreamer = lws_get_protocol(wsi);
		break;

	  /* we get here if a client connects to us */
      case LWS_CALLBACK_ESTABLISHED:
		printf("Restream WS Established\n");
		break;

	  /* if a client closes the connection to us we come in here */
      case LWS_CALLBACK_CLOSED:
		printf("Restream WS Closed\n");
		break;
...
//		printf("Restream WS Writeable\n");
        m = lws_write(wsi, restream_buffer + LWS_PRE, 4097, LWS_WRITE_BINARY);
        if (m < 4097) lwsl_err("WS Error %d  writing to ws\n",m);
		break;

	  /* if a client sends us some data we receive it here */
      case LWS_CALLBACK_RECEIVE:
		printf("Restream WS Receive\n");
